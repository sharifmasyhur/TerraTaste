...
function calculateFootprint() {
  ...
  var carbon_footprints = [
    2.25, 0.434, 0.587, 0.38, 0.2, 0.31, 0.56, 0.006, 0.16, 0.03, 0.11, 0.022,
    0.069, 0.034,
  ];
  // g of Nitrogen lost per serving of each food item
  var nitrogen_footprints = [
    28, 9.93, 11.3, 7.03, 3.49, 4.7, 5.32, 0.378, 0.182, 0.74, 0.53, 0.0028,
    0.7, 0.251,
  ];
  // L of Water consumed per serving of each food item
  var water_footprints = [
    561, 128, 238, 0, 74.1, 168, 165, 70, 23, 100, 130, 44.8, 86, 72.5,
  ];
  ...
  document.getElementById("carbonKG").innerHTML =
    numberWithCommas(annualCF) + "kg";
  document.getElementById("nitrogenG").innerHTML =
    numberWithCommas(annualNF) + "g";
  document.getElementById("waterL").innerHTML =
    numberWithCommas(annualWF) + "L";
...
submitButton.addEventListener("click", function () {
  ...
  firebase.initializeApp({
    apiKey: "AIzaSyCp3wTao3yoQkkN9hsiCmXV9JJc0_RhCng",
    authDomain: "terrataste-109fe.firebaseapp.com",
    databaseURL: "https://terrataste-109fe-default-rtdb.asia-southeast1.firebasedatabase.app",
    projectId: "terrataste-109fe",
    storageBucket: "terrataste-109fe.firebasestorage.app",
    messagingSenderId: "21150331747",
    appId: "1:21150331747:web:21c57788aab52637c11956",
    measurementId: "G-PZH76VGDSR",
  });
...
function showEnvironmentAverageText() {
  ...
  if (annualCF < 680) {
    national_metric = "below";
    needed_metric = "and also below";
  } else if (annualCF < 1750 && annualCF >= 680) {
    national_metric = "below";
    needed_metric = "but above";
  } else if (annualCF < 2250 && annualCF >= 1750) {
    national_metric = "above";
    needed_metric = "and far above";
  } else {
    national_metric = "far above";
    needed_metric = "and also far above";
  }
  ...
  document.getElementById("environ-average-description").innerHTML =
    ...
    "kg of Carbon is " +
    national_metric +
    " the US national per capita average of 1750kg carbon emissions every year<sup>2</sup> " +
    needed_metric +
    " the 680kg maximum upper limit of a sustainable diet necesary to prevent climate catastrophe.<sup>3</sup>";
}

function showEnvironmentText() {
  document.getElementById("environment-description").innerHTML =
    // Carbon
    "For your reference, emitting " +
    numberWithCommas(annualCF) +
    "kgs of Carbon is equivalent to burning " +
    numberWithCommas(Math.round(annualCF / 8.7)) +
    " gallons of gasoline or melting " +
    numberWithCommas(Math.round(annualCF * 0.0165)) +
    " cubic feet of polar ice every year.<sup>4,5</sup> " +
    // Nitrogen
    numberWithCommas(annualNF) +
    "g of Nitrogen waste is equivalent to emitting an additional " +
    numberWithCommas(Math.round((annualNF / 1000) * 298)) +
    "kgs of carbon or polluting an additional " +
    numberWithCommas(Math.round((annualNF * 10) / 453.59)) +
    " pounds of fertilizer every year.<sup>1,4</sup> " +
    // Water
    numberWithCommas(annualWF) +
    "L of water consumption is equivalent to using " +
    numberWithCommas(Math.round(((annualWF / 35) * 0.264172) / 365)) +
    " bathtubs of water every day!<sup>7</sup>";
}
...
var updateSelectButtonValues = function () {
  ...
  beef.on("click", function () {
    foodServings_array[0] = $(this).find("input").val();
  });
  chicken.on("click", function () {
    foodServings_array[1] = $(this).find("input").val();
  });
  pork.on("click", function () {
    foodServings_array[2] = $(this).find("input").val();
  });
  fish.on("click", function () {
    foodServings_array[3] = $(this).find("input").val();
  });
  eggs.on("click", function () {
    foodServings_array[4] = $(this).find("input").val();
  });
  milk.on("click", function () {
    foodServings_array[5] = $(this).find("input").val();
  });
  cheese.on("click", function () {
    foodServings_array[6] = $(this).find("input").val();
  });
  fruits.on("click", function () {
    foodServings_array[7] = $(this).find("input").val();
  });
  vegetables.on("click", function () {
    foodServings_array[8] = $(this).find("input").val();
  });
  wheat.on("click", function () {
    foodServings_array[9] = $(this).find("input").val();
  });
  rice.on("click", function () {
    foodServings_array[10] = $(this).find("input").val();
  });
  oil.on("click", function () {
    foodServings_array[11] = $(this).find("input").val();
  });
  nuts.on("click", function () {
    foodServings_array[12] = $(this).find("input").val();
  });
  beans.on("click", function () {
    foodServings_array[13] = $(this).find("input").val();
  });
};
